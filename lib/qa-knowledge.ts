...
const QA_BASE_KNOWLEDGE_LINES = [
  ...
  "- 部署：可部署到 Netlify / Vercel，导入仓库后直接部署即可（自托管/单机模式默认开启，无需任何环境变量、无需账号门禁）。",
  "- 仓库有 main（正常设备）与 test（兼容设备）两个分支：部分设备全屏或显示异常时部署 test 分支。",
  "- 云端功能（账号、便签墙、游戏大厅、应用市场、黑市等）需要自建 Supabase 并执行 docs/ 下对应建表脚本。",
  "",
  "## 环境变量速查（全部可选，单机模式开关默认即为单机）",
  "- NEXT_PUBLIC_SELF_HOSTED_MODE：默认不设置=单机/自托管模式（跳过账号门禁，无需任何变量）；false=启用账号/激活码门禁（需 Supabase）；true=显式开启单机模式",
