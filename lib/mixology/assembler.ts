...
export type MixAssembleInput = {
    /** The compatibility adapter supplies card/persona/examples and its own format. */
    compatibilityPreset?: boolean;
...
export function assembleMixPrompt(input: MixAssembleInput): MixAssembledPrompt {
    ...
    const baseText = stackBody(m.base?.filter(item => item.compatibility?.format !== "preset"), apply);
    const flavorText = stackBody(m.flavor?.filter(item => item.compatibility?.format !== "worldbook"), apply);
    ...
    const sections: (string | null)[] = [
        ...
        withHung("character", input.compatibilityPreset ? null : sectionBlock(sectionTitle("character"), [
            isMixCardFreeform(card) && mixCardTextHasNameHeading(profileText) ? null : `## ${MIX_CARD_NAME_LABEL}\n${charName}`,
            profileText || null,
        ].map((l) => (l ? apply(l) : l)))),
        // 用户资料：{{user}} 是谁。由面具材料提供，帮模型称呼与理解对面的人
        withHung("persona", !input.compatibilityPreset && persona && persona.content.trim()
            ? [
        ...
        withHung("world", input.compatibilityPreset ? null : sectionBlock(sectionTitle("world"), [worldText || null].map((l) => (l ? apply(l) : l)))),
        withHung("flavor", flavorText ? `# ${sectionTitle("flavor")}\n${flavorText}` : null),
        // 内置协议在前，作者写的正文输出要求接在后面，各自是一个 ## 条目
        withHung("glass", input.compatibilityPreset ? (glassText || null) : `# ${sectionTitle("glass")}\n${PROSE_PROTOCOL}${glassText ? `\n\n## ${sectionTitle("glass")}\n${glassText}` : ""}`),
        ...
        withHung("examples", input.compatibilityPreset ? null : exampleSection(card, charName, userName, sectionTitle("examples"))),
