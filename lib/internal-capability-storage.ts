...
const MUSIC_CONTROL_USAGE_GUIDE = [
    ...
    "- 想直接放给{{user}}听 → 用「播放音乐」工具（真的会在 ta 手机上响起）；只有想'安利/推荐一首歌但不打断当前播放'时，才用 [音乐分享:准确歌名|歌手] 发卡片，必须写明实际歌手，现场版等在歌名中保留版本名，不要编造。后台会匹配歌曲 ID 和封面。{{user}}让你放歌时，默认用工具直接播放，不要只发分享卡片。转账按角色所在地和人设选择币种，格式为 [转账:金额:币种:留言]，外国角色不要无理由使用 CNY。",
    "",
    "动作：播放音乐",
