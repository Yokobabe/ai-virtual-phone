...
function renderParagraph(paragraph: MixProseParagraph, key: number, dialogue?: MixProseDialogue, safeHtml?: boolean) {
    // HTML 片段：沙盒框就地渲染（与开场画布、尾调同一个框）；代码块：等宽显示
    if (paragraph.type === "html") {
        return <div className="mix-html-block" key={key}><MixRichText text={paragraph.html} safeHtml={safeHtml} /></div>;
    }
...
export function MixProseView({ text, dialogue, streaming, safeHtml }: { text: string; dialogue?: MixProseDialogue; streaming?: boolean; safeHtml?: boolean }) {
    const paragraphs = useMemo(() => parseMixProse(text, { streaming }), [text, streaming]);
    return (
        <div className="mix-prose">
            {paragraphs.map((paragraph, i) => renderParagraph(paragraph, i, dialogue, safeHtml))}
        </div>
    );
}
