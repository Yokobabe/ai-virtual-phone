...
import { CompatibilityPresetEditor } from "./compatibility-preset-editor";
import { CompatibilityDataEditor } from "./compatibility-data-editor";
import { record, records, string } from "@/lib/mixology/compatibility";
...
export function MixMaterialEditor({ kind, initial, onSave, onCancel }: EditorProps) {
    ...
    const handleSave = () => {
        try { for (const rule of records(record(compatibility?.data.extensions).regex_scripts)) if (!rule.disabled && string(rule.findRegex)) parseCompatibilityRegex(string(rule.findRegex)); }
        catch { setError("有启用的正则表达式语法错误，请修正或停用后保存。"); return; }
        ...
        if ((kind === "base" && compatibility?.format === "preset") || (kind === "flavor" && compatibility?.format === "worldbook")) {
            onSave({ ...meta, kind, content: "" });
            return;
        }
        if (!content.trim()) {
    ...
    return (
        <div>
            ...
            {compatibility ? <CompatibilityDataEditor value={compatibility} onChange={setCompatibility} /> : null}
            {kind === "base" && compatibility?.format === "preset" ? <CompatibilityPresetEditor data={compatibility.data} onChange={data => setCompatibility({ ...compatibility, data })} /> : null}
            {(kind === "preface" || kind === "base" || kind === "flavor" || kind === "glass" || kind === "strength" || kind === "checklist") && compatibility?.format !== "preset" && compatibility?.format !== "worldbook" ? (
