"use client";
import { CompatibilityProse } from "./compatibility-prose";
...
function AssistantTurn({ turn, ticketFrames, encoreFrames, filterRules, state, dialogue, compatibility }: { compatibility: Omit<CompatibilityTextInput, "text">; turn: MixTurn; ticketFrames: TurnFrame[]; encoreFrames: TurnFrame[]; filterRules?: MixFilterRule[]; state?: MixState; dialogue?: MixProseDialogue }) {
    ...
    return (
        <>
            ...
            {shownText ? <CompatibilityProse input={{ ...compatibility, text: shownText }} dialogue={dialogue} /> : null}
            {turn.compatibilityLoreReport?.entries.length ? <details className="mix-compat-prompt"><summary>本轮世界书 · 约 {turn.compatibilityLoreReport.estimatedTokens} tokens</summary><ul>{turn.compatibilityLoreReport.entries.map((e, i) => <li key={i}>{e.name}：{e.status}</li>)}</ul></details> : null}
...
export function MixologyGame({ sessionId, onBack, onToast }: GameProps) {
    ...
    const assets = useMemo(() => {
        if (!session) return { compatibilityActive: {} as CompatibilityTextInput["active"], cover: "", tickets: [] as { id: string; html: string }[], garnishCss: "", aiEncores: [] as { id: string; html: string }[], encoreStatics: [] as { id: string; html: string }[], canvasHtml: "", filterRules: undefined as MixFilterRule[] | undefined };
                ...
                {session.turns.slice(hiddenTurnCount).map((turn, offset) => {
                    ...
                    return turn.role === "user" ? (
                        <div className="mix-user-turn" data-with-actions="true" data-turn-id={turn.id} key={turn.id}>
                            <div className="mix-user-bubble"><CompatibilityProse input={{ text: turn.text, active: assets.compatibilityActive, charName: session.charName, userName: session.userName || "你", phase: "display", placement: 1, depth: session.turns.length - idx - 1, isEdit: turn.compatibilityEdited }} /></div>
                            {actions}
                        </div>
                    ) : (
                        <div className="mix-assistant-turn" key={turn.id} data-turn={turn.id} data-turn-id={turn.id}>
                            <AssistantTurn compatibility={{ active: assets.compatibilityActive, charName: session.charName, userName: session.userName || "你", phase: "display", placement: 2, depth: session.turns.length - idx - 1, isEdit: turn.compatibilityEdited }} turn={turn} ticketFrames={turnTicketFrames(turn)} encoreFrames={turnEncoreFrames(turn)} filterRules={assets.filterRules} state={turn.state} dialogue={dialogueFor(turn.id)} />
                ...
                {busy ? (() => {
                    ...
                    return shown ? (
                        <div className="mix-live-turn">
                            <CompatibilityProse input={{ text: shown, active: assets.compatibilityActive, charName: session.charName, userName: session.userName || "你", phase: "display", placement: 2, depth: 0, streaming: true }} />
                        </div>
                    ) : (
